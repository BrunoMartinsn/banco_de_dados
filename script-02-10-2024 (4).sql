...
-- selecionar pizzas que começa com a letra A like:parte do texto
select * from pizzas where nome like '%a';

-- selecionar pizzas que começa com a letra L like:parte do texto
...
-- Questão 7
select nome, valor from clientes left join pedidos on clientes.id = pedidos.cliente_id group by nome,valor;

-- Questão 8
select clientes.nome, pedidos.id as pedido, pedidos.data, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id = clientes.id where pedidos.data between 
...
-- Questão 14
select pedidos.id as cliente, nome, pedidos.id as pedido, max(valor) from clientes left join pedidos on pedidos.cliente_id = clientes.id;
select * from pedidos;
...
-- Questão 16
select sum(quentidade) as total_pizzas, count(pedidos.id) as total_pedidos from itens_pedido
inner join pedidos on itens_pedido.pedido_id = pedidos.id
inner join clientes on pedidos.cliente_id = clientes.id
where clientes.nome = 'Bruna Dantas';
-- Questão 17
select pedidos.id as clientes,nome,pedidos.id as pedido, max(valor), min(valor) from clientes left join pedidos on 
pedidos.cliente_id = clientes.id where clientes.nome = 'Laura Madureira';
...
-- Questão 19
select pedidos.id as cliente, nome, pedidos.id as pedido, min(valor) from clientes left join pedidos on pedidos.cliente_id = clientes.id;

-- Questão 20
select clientes.nome,  count(pedidos.id) as total_pedidos from pedidos right join clientes on 
clientes.id group by clientes.id, clientes.nome ;

-- Questão 21

-- Questão 22
select nome, pedidos.id as pedido, pedidos.valor from clientes inner join pedidos on pedidos.cliente_id
 = clientes.id order by valor desc limit 3;
